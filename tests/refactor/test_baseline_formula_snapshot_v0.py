"""
tests/refactor/test_baseline_formula_snapshot_v0.py
====================================================
FORMULA_REFACTOR_BASELINE_0 — Guardrails (TAREA 4).

Estos tests congelan el comportamiento numérico del motor NEXA ANTES de
reorganizar fórmulas. Protegen contra drift introducido por el refactor:
  ...
  3. test_snapshot_bitwise_parity    — output == snapshot congelado (bit a bit)
  4. test_kpis_anchor_values         — anclas numéricas explícitas de KPIs

NO modifican código productivo. Si el refactor cambia un valor, estos tests
fallan y obligan a investigar el drift (regla crítica: no actualizar golden
...
SNAPSHOT_PATH = Path(__file__).resolve().parent / "baseline_formula_snapshot_v0.json"

SIMULATION_ID = "baseline_formula_v0"
...
def test_snapshot_parity(engine_output, snapshot):
    ...
    assert _normalize(full) == _normalize(snapshot), (
        "DRIFT detectado vs snapshot baseline_formula_snapshot_v0.json. "
...
def test_kpis_anchor_values(engine_output):
    ...
    # CAPEX valor_mes 6250000→6345625, volumetria Voz2 cadena_b 1→100000, etc.
