...
def _build_scenario_entry(meta: dict, raw: dict, channel: dict, vt_data: Optional[dict] = None) -> dict:
    ...
    entry = {
        ...
        "componente_fijo": (
            ...
            if "componente_fijo_label" in meta
            else _resolve_componente_label(meta, channel)
        ),
    ...
    for i in range(1, 6):
        ...
        if scenario_id in scenario_map:
            ...
            rows.append({
                ...
                "proporcion_componente_fijo_pct": entry.get("proporcion_componente_fijo_pct", 0),
                "componente_variable": entry.get("componente_variable"),
                "proporcion_componente_variable_pct": entry.get("proporcion_componente_variable_pct", 0),
                "facturacion_directa": entry.get("facturacion_directa", 0),
...
def _empty_scenario_row(scenario_number: str) -> dict:
    return {
        ...
        "proporcion_componente_variable_pct": 0,
        "facturacion_directa": 0,
    ...
    for i in range(1, 6):
        ...
        if scenario_id in scenario_map:
            ...
            items.append({
                ...
                "honorarios_totales": source.get("honorarios_totales") or [],
                "ventas_multicanal": source.get("ventas_multicanal") or [],
                "desglose_componente_fijo": source.get("desglose_componente_fijo") or {},
...
def _build_total_detail(
    ...
    return {
        ...
        "honorarios_cobranza": (escenario_total.get("honorarios_cobranza") if escenario_total else None) or [],
        "honorarios_totales": (escenario_total.get("honorarios_totales") if escenario_total else None) or [],
        "ventas_multicanal": (escenario_total.get("ventas_multicanal") if escenario_total else None) or [],
        "desglose_componente_fijo": (escenario_total.get("desglose_componente_fijo") if escenario_total else None) or {},
        "fte": fte_total,
...
def _build_reglas_negocio(reglas: dict, root_rules: dict) -> dict:
    return {
        ...
        "descuento_cliente": _safe_float(_coalesce(
            reglas.get("descuento_volumen"), reglas.get("descuento")
        )),
        "contingencia_operativa": _safe_float(reglas.get("cont_operativa")),
        "contingencia_comercial": _safe_float(reglas.get("cont_comercial")),
