...
class UserInputLoader(
    ...
    def _normalizar_entry_data_format(data: Dict) -> Dict:
        ...
        panel_de_control = {
            ...
            "descuento":           float((reg.get("descuento_volumen") or {}).get("valor", 0.0) if isinstance(reg.get("descuento_volumen"), dict) else float(reg.get("descuento_volumen") or 0.0)),
            "margen_b":            float(reg["margen_objetivo_cadena_b"]) if reg.get("margen_objetivo_cadena_b") is not None else None,
            "margen_c":            float(reg["margen_objetivo_cadena_c"]) if reg.get("margen_objetivo_cadena_c") is not None else None,
            "tasa_ica":            ops.get("tasa_ica", 0.0),
            "tasa_gmf":            ops.get("tasa_gmf", 0.0),
            "activa_financiacion": ops.get("cons_costo_de_financiacion", True),
            "antiguedad_cliente":  ops.get("antiguedad_cliente") or ops.get("antiguedad", ""),
            "periodo_pago_dias":   int(ops.get("periodo_pago", 90)),
