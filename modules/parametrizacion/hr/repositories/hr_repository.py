...
class HRRepository:
    ...
    def save_version(self, summary: VersionSummary, data: dict, metadata: dict | None = None) -> str:
        """Guarda una versión HR con status=active y desactiva la versión activa anterior.

        La búsqueda de registros activos previos se hace via query directa al store
        (domain=hr, status=active) para que funcione tanto con Cosmos como con JSON store.
        """
        domain_name = HR_PARAMETRIZATION_COLLECTION.name  # "hr"
    ...
    def _query_active_ids(self, domain_name: str) -> list:
        ...
        Estrategia:
        1. query(domain=hr, status=active) — funciona en Cosmos y JSON store (con domain en payload)
    ...
    def list_versions(self) -> List[VersionSummary]:
        ...
        domain_name = HR_PARAMETRIZATION_COLLECTION.name

        try:
            docs, _ = self._store.query(
                HR_PARAMETRIZATION_COLLECTION, {"domain": domain_name}
            )
            if docs:
                summaries = []
                for doc in docs:
    ...
    def get_document_raw(self, version_id: str) -> dict:
        """Obtiene el documento completo de Cosmos filtrando por domain='hr' e id."""
        domain_name = HR_PARAMETRIZATION_COLLECTION.name
        _sys_keys = {"_rid", "_self", "_etag", "_attachments", "_ts"}
        docs, _ = self._store.query(
            HR_PARAMETRIZATION_COLLECTION, {"domain": domain_name, "id": version_id}
        )
        if not docs:
            raise NotFoundError("version", version_id)
    ...
    def activate_version(self, version_id: str) -> VersionSummary:
        ...
        docs, _ = self._store.query(HR_PARAMETRIZATION_COLLECTION, {"domain": domain_name})
    ...
    def get_active_record(self) -> tuple[Optional[VersionSummary], Optional[dict]]:
        ...
        Estrategia:
        1. query(domain=hr, status=active)  — consulta directa Cosmos (principal)
        2. índice filesystem               — fallback solo para DB_PROVIDER=json local
        3. query(domain=hr)                — Cosmos fallback para docs sin campo status
        """
        domain_name = HR_PARAMETRIZATION_COLLECTION.name  # "hr"
        _sys_keys = {"id", "domain", "payload", "_rid", "_self", "_etag", "_attachments", "_ts"}
        ...
        # Step 1: Cosmos — WHERE domain='hr' AND status='active'
        try:
            docs, _ = self._store.query(
                HR_PARAMETRIZATION_COLLECTION, {"domain": domain_name, "status": "active"}
            )
            if docs:
        ...
        try:
            ...
            if active_summary is not None:
                record = self._store.get_record(HR_PARAMETRIZATION_COLLECTION, active_summary.version_id)
                if record is not None:
                    return active_summary, self._codec.decode(record)
        except Exception:
            pass

        # Step 3: Cosmos fallback — WHERE domain='hr' (docs sin campo status)
        try:
            docs, _ = self._store.query(HR_PARAMETRIZATION_COLLECTION, {"domain": domain_name})
            if docs:
