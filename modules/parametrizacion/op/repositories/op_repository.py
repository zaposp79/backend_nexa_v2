...
class OPRepository:
    ...
    def list_versions(self) -> List[VersionSummary]:
        ...
        try:
            docs, _ = self._store.query(
                OP_PARAMETRIZATION_COLLECTION, {"domain": domain_name}
            )
            if docs:
                summaries = []
                for doc in docs:
    ...
    def get_active_record(self) -> tuple[Optional[VersionSummary], Optional[dict]]:
        ...
        Estrategia:
        1. query(domain=op, status=active)  — consulta directa Cosmos (principal)
        2. índice filesystem               — fallback solo para DB_PROVIDER=json local
        3. query(domain=op)                — Cosmos fallback para docs sin campo status
        """
        ...
        # Step 1: Cosmos — WHERE domain='op' AND status='active'
        try:
            docs, _ = self._store.query(
                OP_PARAMETRIZATION_COLLECTION, {"domain": domain_name, "status": "active"}
            )
            if docs:
        ...
        try:
            ...
            if active_summary is not None:
                record = self._store.get_record(OP_PARAMETRIZATION_COLLECTION, active_summary.version_id)
                if record is not None:
                    return active_summary, self._codec.decode(record)
        except Exception:
            pass

        # Step 3: Cosmos fallback — WHERE domain='op' (docs sin campo status)
        try:
    ...
    def _deactivate_in_store(self, version_id: str) -> None:
        try:
            ...
            self._store.upsert_record(OP_PARAMETRIZATION_COLLECTION, updated)
        except Exception:
            pass
